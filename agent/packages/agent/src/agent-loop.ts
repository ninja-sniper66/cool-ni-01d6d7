...
async function runLoop(
	...
	const parseAcceptanceCriteriaBullets = (text: string): string[] => {
		...
		for (const line of lines) {
			...
			// Indented line = continuation of previous item
			if (out.length > 0 && line.length > 0 && /^\s+/.test(line) && t.length) {
				out[out.length - 1] = (out[out.length - 1] + " " + t).trim();
				continue;
			}
			// First non-bullet, non-continuation after list start → end of acceptance block
			if (out.length > 0) {
				break;
			}
		}
	...
	const pushSteeringNudge = (id: string, minIntervalMs: number, msg: AgentMessage): void => {
		...
		if (process.env.AGENT_LOOP_NUDGE_DEBUG === "1") {
			// Non-fatal: helps A/B and regression tracking only.
			console.error(`[agent-loop] nudge_fired id=${id}`);
		}
	};
	...
	const recordSuccessfulFileMutation = async (targetPath: string): Promise<void> => {
		...
		try {
			...
			const related = siblings
				...
				.slice(0, isNarrowTask ? 3 : 8);
			if (related.length > 0) {
				if (isNarrowTask) {
					// v233: do not inflate foundFiles from neighbors on tight tasks (reduces wrong-file diffs)
					siblingHint = ` Related in same directory (open only if the task needs them): ${related.map((f: string) => `\`${f}\``).join(", ")}.`;
				} else {
		...
		const complianceTail =
			acceptanceCriteriaBullets.length > 0
				? " Re-read the task: each acceptance line must hold in the codebase (including negatives: 'must not render', 'no longer', etc. — not only one mechanism like a redirect)."
				: "";
		pendingMessages.push({
			...
			content: [
				{
					type: "text",
					text: `\`${targetPath}\` updated successfully.${breadthHint}${siblingHint}${complianceTail}`,
				},
			],
			timestamp: Date.now(),
		});
	...
	while (true) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			// v217 / v233: slow-pace — 10s avg, with cooldown to avoid piling on with other nudges
			if (totalLlmRequests >= 3 && pendingMessages.length === 0) {
				const elapsed = Date.now() - loopStart;
				const avgPace = elapsed / totalLlmRequests;
				// Slightly stricter for narrow tasks: wall-clock is precious on small file sets
				const paceMax = isNarrowTask ? 8_000 : 10_000;
				if (avgPace > paceMax && elapsed > (isNarrowTask ? 12_000 : 20_000)) {
					const topFile = foundFiles[0] || [...pathsAlreadyRead][0] || "";
					pushSteeringNudge("slow-pace", isNarrowTask ? 25_000 : 30_000, {
						role: "user",
						content: [
							{
								type: "text",
								text: `You are averaging ${Math.round(avgPace / 1000)}s per response — too slow. Shorten reasoning and call tools in parallel where safe. ${
									topFile && !hasProducedEdit ? "Call `edit` on `" + topFile + "` NOW." : "Keep editing; verify each stated criterion is satisfied."
								}`,
							},
						],
						timestamp: Date.now(),
			...
			// v222 / v233: low-edit coverage on broad tasks only (broad nudges hurt single-file quality)
			if (hasProducedEdit && !isNarrowTask && pendingMessages.length === 0) {
				const elapsed = Date.now() - loopStart;
				const uneditedTop = foundFiles.filter((f: string) => !wasEdited(f));
				if (elapsed >= 60_000 && foundFiles.length >= 3 && editedPaths.size < foundFiles.length && uneditedTop.length > 0 && elapsed < GRACEFUL_EXIT_MS - 30_000) {
					const ratio = editedPaths.size / Math.max(foundFiles.length, 1);
					// Slightly lower threshold to catch "almost there" under-judge faster
					if (ratio < 0.55) {
						const list = uneditedTop.slice(0, 5).map((f: string) => `\`${f}\``).join(", ");
						pushSteeringNudge("low-coverage", 50_000, {
							role: "user",
							content: [
								{
									type: "text",
									text: `WARNING: ${Math.round(elapsed / 1000)}s elapsed but you have only edited ${editedPaths.size}/${foundFiles.length} target files. Unedited: ${list}. Each unedited file risks missed points. Read and edit them now — even partial changes count. Do not park on one file if others stay untouched.`,
								},
							],
							timestamp: Date.now(),
						});
			...
			// v235: full task prompt — implement every line under "Acceptance criteria" (any task; no domain-specific paths)
			if (!hasMoreToolCalls && hasProducedEdit && acceptanceCriteriaBullets.length > 0 && taskPromptComplianceRetries < MAX_TASK_PROMPT_COMPLIANCE) {
				taskPromptComplianceRetries++;
				await emit({ type: "turn_end", message, toolResults: [] });
				const numbered = acceptanceCriteriaBullets.map((b, j) => `${j + 1}. ${b}`).join("\n");
				pendingMessages.push({
					role: "user",
					content: [
						{
							type: "text",
							text: `A partial implementation (e.g. only a redirect) is not enough. The full task text applies: you must fulfill **each** line under the acceptance / requirements copy in the task, including compound and negative lines ("must not", "should not", "no longer", "do not render" — that usually means removing or replacing UI/code, not only adding a route elsewhere).

Checklist from the task — confirm each in the repo, then \`read\` / \`edit\` as needed:

${numbered}

If a line says both "redirect" and "must not show X at /", you typically need the redirect **and** the page/route must not still render the old X. If every line is satisfied, reply "done".`,
						},
					],
					timestamp: Date.now(),
				});
			...
			if (hasMoreToolCalls) {
				...
				if (workPhase === "search") {
					...
					if (workPhase !== "apply") {
						// v233: fewer mandatory reads on 1–2 file tasks → faster first edit, less drift
						const absorbLimit = isNarrowTask
							? Math.max(1, Math.min(3, foundFiles.length <= 1 ? 1 : foundFiles.length))
							: Math.min(Math.max(3, foundFiles.length > 10 ? 6 : 3), 8);
				...
				for (let i = 0; i < toolResults.length; i++) {
					...
					if (tr.toolName === "read" && !tr.isError && tc && tc.type === "toolCall") {
						...
						if (readPath && typeof readPath === "string" && isRealUnixFile(readPath)) {
							...
							const maxFoundCap = isNarrowTask ? 12 : 25;
							const maxImportFollows = isNarrowTask ? 6 : 40;
							if (readContent.length > 0 && readContent.length < 200_000 && foundFiles.length < maxFoundCap && importFollowAdds < maxImportFollows) {
								...
								while ((im = importRe.exec(readContent)) !== null) {
									if (foundFiles.length >= maxFoundCap || importFollowAdds >= maxImportFollows) break;
									const raw = im[1];
									let norm: string;
									if (raw.startsWith("@/")) {
				...
				if (now - lastRereadNudgeAt >= 5_000 && pendingMessages.length === 0) {
					for (const [rp, cnt] of pathReadCounts) {
						if (cnt >= 3) {
							...
							pushSteeringNudge("reread", 7_000, {
								role: "user",
								content: [
									{
										type: "text",
				...
				const dynamicExploreCeiling = isNarrowTask
					? Math.max(2, Math.min(foundFiles.length + 1, 4))
					: Math.max(3, Math.min(foundFiles.length + 1, 6));
				if (!hasProducedEdit && explorationCount >= dynamicExploreCeiling && pendingMessages.length === 0) {
					pushSteeringNudge("explore-first", isNarrowTask ? 8_000 : 12_000, {
						role: "user",
						content: [
							{
								type: "text",
				...
				if (
					!hasProducedEdit &&
					totalExplorationSteps >= (isNarrowTask ? 4 : 5) &&
					pendingMessages.length === 0 &&
					foundFiles.length > 0
				) {
					const primary = foundFiles[0].replace(/^\.\//, "");
					pushSteeringNudge("discovery-stall", isNarrowTask ? 14_000 : 18_000, {
						role: "user",
						content: [
							{
								type: "text",
				...
				// FORCE EDIT: 45s+ with no edit — still critical; v233 throttles repeat spam
				if (!hasProducedEdit && (Date.now() - loopStart) >= 45_000 && pathsAlreadyRead.size > 0) {
					const topFile = foundFiles[0] || [...pathsAlreadyRead][0] || "";
					if (topFile) {
						const lastF = nudgeLastFired.get("force-zero-edit") ?? 0;
						if (Date.now() - lastF >= 12_000) {
							nudgeLastFired.set("force-zero-edit", Date.now());
							pendingMessages = [{
								role: "user",
								content: [{
									type: "text",
		...
		if (!reviewPassDone && hasProducedEdit && reviewElapsed < 60_000) {
			...
			const hint = (() => {
				if (acceptanceCriteriaBullets.length > 0) {
					const list = acceptanceCriteriaBullets
						.slice(0, 8)
						.map((b, j) => `${j + 1}) ${b}`)
						.join(" | ");
					return isNarrowTask
						? uneditedTargets.length > 0
							? `Cross-check each: ${list}. You still have unedited candidates ${uneditedTargets
									.slice(0, 3)
									.map((f: string) => `\`${f}\``)
									.join(", ")} — only if a line still requires it.`
							: `Verify each: ${list} — every line, including "must not" and behavior+redirect pairs.`
						: uneditedTargets.length > 0
							? `Per item: ${list}. Also read/edit: ${uneditedTargets.slice(0, 5).map((f: string) => `\`${f}\``).join(", ")}.`
							: `Per item: ${list}. Grep the repo for anything a line said must vanish.`;
				}
				if (isNarrowTask) {
					return uneditedTargets.length > 0
						? `Optional: you have not changed ${uneditedTargets.slice(0, 4).map((f: string) => `\`${f}\``).join(", ")} — touch them only if a criterion or named path still requires it.`
						: `Re-check each requirement in the task. Grep for any string the task said must change. If all are met, reply "done".`;
				}
				if (uneditedTargets.length > 0) {
					return `Unedited discovered files: ${uneditedTargets.slice(0, 5).map((f: string) => `\`${f}\``).join(", ")}. Read and edit them.`;
				}
				return `Re-read the task. If the task listed exact old strings or labels, grep the repo for any that remain. If you missed a requirement, edit; if not, reply "done".`;
			})();
			pendingMessages = [{
				role: "user",
